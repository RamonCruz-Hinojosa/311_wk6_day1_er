...
1. What is the relationship between the "actor" and "film_actor" tables?
   ...
   Going from the actor table to the film_actor table theres a one and only one connection. the actor can only be in a film once but can be in multiple different films.

2. What does the blue diamond next to the "last_update" column on the "inventory" table represent?

The blue diamond next to last_update means not null simple attribute.

3. How many foreign keys does the "payments" table have? How can you tell?

   It looks like there are at least 3 foreign keys in the payment table. Any symbol that is red represents a foreign key.
